...
/*  SEPARATE CODE FROM THE MAIN LIPSYNC FIRMWARE  */
...
void stretch_calculation(int x_UP, int x_RIGHT, int x_DOWN, int x_LEFT, int y_UP, float yy_RIGHT, float yy_DOWN, float yy_LEFT);
...
void loop(){
  ...
  yh_stretch = stretch_calculation(x_UP, x_RIGHT, x_DOWN, x_LEFT, y_UP, y_RIGHT, y_DOWN, y_LEFT);
...
void theta_calculation(int x_UP, int y_UP) {                             //calculates angle 'theta' to rotate axis by, based on the comfortable 'UP' position
  ...
  if((y_UP > 0) && (x_UP < 0)){         //quadrant 2
    theta = atan(x_UP/y_UP);
  }
  
  else if((y_UP < 0) && (x_UP < 0)){    //quadrant 3
    theta = atan(y_UP/x_UP) + pi/2;
  }

  else if((y_UP < 0) && (x_UP > 0)){    //quadrant 4
    theta = atan(y_UP/x_UP) + pi;
  }

  else if((y_UP > 0) && (x_UP > 0)){    //quadrant 1
    theta = 2*pi - atan(x_UP/y_UP);
  }


}

void stretch_calculation(float xx_UP, float xx_RIGHT, float xx_DOWN, float xx_LEFT, float yy_UP, float yy_RIGHT, float yy_DOWN, float yy_LEFT){
  ...
  magnitude_up_funct = sqrt(pow(xx_UP, 2) + pow(yy_UP, 2));
  magnitude_right_funct = sqrt(pow(xx_RIGHT,2) + pow(yy_RIGHT,2));
  magnitude_down_funct = sqrt(pow(xx_DOWN,2) + pow(yy_DOWN,2));
  magnitude_left_funct = sqrt(pow(xx_LEFT,2) + pow(yy_LEFT,2));
